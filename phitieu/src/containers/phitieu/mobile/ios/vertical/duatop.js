...
import bg_page_sanqua from '../../images/bg_duatop_dung.png';
...
class Lucky_Rotation extends React.Component {
		...
		if(none_multi){
			if(isPlay){
				if(countDart>0){
					...
					// 	this.fd(touchPos.x + arr[0] -1, touchPos.y-heightFrame/2 + height_plus + arr[1])
					// }else{
					// 	this.showTextWarning()
	...
	autoPlay=()=>{
		const {tieuconlai, txt_points, awardsContent, dartFlightImg, isChangetab, countDart, hight_score, sessionId, code_key, height_plus, delta}=this.state;
		var user = JSON.parse(localStorage.getItem("user"));
		curFrame=0;
		var plus=heightFrame/2 - height_plus
		if(countDart>0){
			...
			this.props.gds(2,sessionId, user.Token, code_key, "iosp", 0, 0, 0, delta).then(()=>{
				var data=this.props.dataUserSpin;
				if(data.Status===0){
					if(!isChangetab){
	...
	render() {
		...
		const {msg, vertical, auto_play, sanqua, msg_err}=this.state;
		...
		return (
				<div id="game" style={{backgroundColor:'#000000'}}>
						...
						<div id="div_checkbox" style={{position:'absolute', top:width_bgImg*0.88, left:"5%", zIndex:999999}} onTouchStart={this.check_auto}></div>
