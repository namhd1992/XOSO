...
class Lucky_Rotation extends React.Component {
		...
		if(none_multi){
			if(isPlay){
				if(countDart>0){
					...
					// }
					this.props.gds(2,sessionId, user.Token, code_key, "iosl", x, y, 54, delta).then(()=>{
	...
	autoPlay=()=>{
		const {tieuconlai, txt_points, awardsContent,dartFlightImg,hight_score, isChangetab, countDart,  sessionId, code_key, delta}=this.state;
		var user = JSON.parse(localStorage.getItem("user"));
		curFrame=0;
		if(countDart>0){
			if(JSON.stringify(dartFlightImg) !== '{}'){
			...
			this.props.gds(2,sessionId, user.Token, code_key, "iosl", 0, 0, 0, delta).then(()=>{
				var data=this.props.dataUserSpin;
				if(data.Status===0){
					if(!isChangetab){
