...
// http://10.1.0.231:3000/?auth-code=VUJPMXVTQTl6dWswVVgvUzZySUc5dUpwL2p3YUlhakJrUklmeW5kZE9GdWtlUXlTQmM3TDVUdys2WXU2SC9SbUtldFRIdi94bUU5NHlpcURBMjUzanhlRllaWUlZZz09'
...
var totalScore = 0;
var p_h=window.screen.height - window.innerHeight;
var p_w=window.screen.width - window.innerWidth;
...
if(deltal_device > deltal_img){
	bg_y=height;
	bg_x=(width*deltal_img/deltal_device);
	mg_l=p_h/2;
	delta=window.innerWidth/img_w;
}else{
	bg_x=width;
	bg_y=height*deltal_device/deltal_img;
	mg_t=p_w/2;	
	delta=window.innerHeight/img_h;
}
// alert(p_w)
...
class Lucky_Rotation extends React.Component {
	...
	touchEnd=(e)=>{
		...
		const {tieuconlai, txt_points, awardsContent,hight_score,  stage, darthVaderImg, dartPositionY, isPlay, none_multi, countDart,  sessionId, auto_play, code_key, delta}=this.state;
		var user = JSON.parse(localStorage.getItem("user"));
		var _this=this;
		var arr=[];
		if(none_multi){
	...
	autoPlay=()=>{
		const {isVideo, tieuconlai, txt_points, awardsContent,dartFlightImg,hight_score, isChangetab, countDart,  sessionId, code_key, delta}=this.state;
		var user = JSON.parse(localStorage.getItem("user"));
		curFrame=0;
		if(countDart>0){
			if(JSON.stringify(dartFlightImg) !== '{}'){
			...
			this.props.gds(2,sessionId, user.Token, code_key, "android", 0, 0, 0, delta).then(()=>{
				var data=this.props.dataUserSpin;
				if(data.Status===0){
					if(!isChangetab){
	...
	render() {
		...
		return (
				<div id="game" style={{height:"100%", backgroundColor:"black"}}>
					<div>{(horizontal)?(<div>
						{(auto_play)?(<div id="canvas" style={{top:0, left:0 + mg_l, zIndex:99999, backgroundColor:"black"}}></div>):(<div id="canvas" style={{position:'absolute', top:0, left:0, zIndex:99999}} onTouchStart={(e) =>this.touchStart(e)} onTouchEnd={(e)=>this.touchEnd(e)} onTouchMove={(e)=>this.touchMove(e)}></div>)}
						<div id="div_checkbox" style={{position:'absolute', top:height_bgImg*0.83, left:mg_l, zIndex:999999}} onTouchStart={this.check_auto}></div>
						<div id="div_exit" style={{position:'absolute', top:0, left:"83%", zIndex:999999}} onTouchStart={this.exit}></div>
						</div>):(<div>
